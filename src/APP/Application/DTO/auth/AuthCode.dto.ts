...
export class AuthCodeDto extends BaseDto {
  ...
  constructor(readonly authuser: string, readonly hd: string, readonly prompt: string, readonly scope: string, readonly code: string) {
    super();
  }

  static create(data: unknown): DtoValidationResult<AuthCodeDto> {
    const res = BaseDto.validate<{ authuser: string, hd: string, prompt: string, scope: string, code: string }>(AuthCodeDto.schema, data);
